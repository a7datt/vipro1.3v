...
async function processBotOrder(chatId: number, user: any, product: any, price: number, extraData: any) {
  try {
    ...
    if (user.referred_by_id) {
      ...
      const refCommRate1 = refCommSetting1 ? (parseFloat(refCommSetting1.value) || 10) / 100 : 0.05;
  ...
  app.post("/api/orders", authenticate, orderLimiter, async (req, res) => {
    try {
      ...
      if (user.referred_by_id) {
        ...
        const refCommRate2 = refCommSetting2 ? (parseFloat(refCommSetting2.value) || 10) / 100 : 0.05;
