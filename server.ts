...
async function startServer() {
  ...
  app.post("/api/orders", authenticate, orderLimiter, async (req, res) => {
    try {
      ...
      if (hasExternalId && orderMode === 'auto') {
        ...
        // استخراج playerId فقط إذا كان المنتج يتطلب بيانات (requires_input)
        // المنتجات مثل بطاقات الأرقام الأوروبية لا تتطلب playerId
        const playerId = product.requires_input ? (
          extraData?.playerId ||
          extraData?.input ||
          extraData?.userId ||
          extraData?.gameId ||
        ...
        if (product.requires_input && !playerId) {
  ...
  app.get("/api/orders/user/:userId", authenticate, async (req, res) => {
    // تأكد أن المستخدم يشوف طلباته هو فقط
    if (String((req as any).userId) !== String(req.params.userId)) {
      return res.status(403).json({ error: "ممنوع" });
    }
    try {
  ...
  const autoSyncProducts = async () => {
    ...
    try {
      ...
      // جلب كل منتجات API الموجودة في قاعدة البيانات
      const { data: dbProducts } = await supabase
        .from("products")
        .select("id, external_id, available")
      ...
      let updated = 0, deleted = 0, restored = 0;
      ...
      for (const dbProd of (dbProducts || [])) {
        ...
        if (!apiProd) {
          // المنتج غير موجود في API أصلاً → احذفه
        ...
        if (!apiAvailable && dbProd.available) {
          // أصبح غير متوفر → احذفه
          await supabase.from("products").delete().eq("id", dbProd.id);
          deleted++;
        } else if (apiAvailable && !dbProd.available) {
          // أصبح متوفراً → أعده
          await supabase.from("products").update({ available: true, name: apiProd.name }).eq("id", dbProd.id);
          restored++;
        } else if (apiAvailable) {
          // حدّث الاسم فقط (السعر يحافظ عليه الأدمن)
          await supabase.from("products").update({ name: apiProd.name }).eq("id", dbProd.id);
          updated++;
        }
      }

      console.log(`[AUTO-SYNC] اكتملت: تحديث=${updated}, حذف=${deleted}, استعادة=${restored}`);
    } catch (e: any) {
      console.error("[AUTO-SYNC] خطأ:", e.message);
    }
  };

  // تشغيل فوري عند بدء السيرفر ثم كل ساعة
