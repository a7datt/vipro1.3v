...
async function startServer() {
  ...
  app.get("/api/referrals/stats/:userId", authenticate, async (req, res) => {
    ...
    try {
      ...
      // تحديث referral_count في users و user_stats ليكونا متزامنَين
      if (actualCount > 0) {
  ...
  app.post("/api/orders", authenticate, orderLimiter, async (req, res) => {
    try {
      ...
      if (hasExternalId && orderMode === 'auto') {
        ...
        // استخراج playerId
        // إذا المنتج يتطلب input → يُؤخذ من extraData
        // إذا لا → نستخدم رقم الهاتف أو ID المستخدم كـ fallback تلقائي
        let playerId = "";
        if (product.requires_input) {
          ...
          // التحقق: إذا المنتج يتطلب input لكن لم يُرسَل
          if (!playerId) {
            return res.status(400).json({ error: "هذا المنتج يتطلب معرف اللاعب (Player ID)" });
          }
        } else {
          // المنتج بدون حقل مدخل → نرسل رقم الهاتف أو ID تلقائياً
          playerId = (
            extraData?.playerId ||
            extraData?.input ||
            user.phone ||
  ...
  app.get("/api/orders/check-external/:orderId", authenticate, async (req, res) => {
    try {
      ...
      const userId = (req as any).userId;
      // تأكد أن طلبات هذا المستخدم تحتوي على هذا الـ orderId في الـ meta
      const { data: orders } = await supabase
        .from("orders")
        .select("id, meta")
        .eq("user_id", userId)
        .eq("status", "processing");
      const owns = (orders || []).some((o: any) => {
        try { const m = JSON.parse(o.meta || "{}"); return String(m.ahminix_order_id) === String(orderId); }
        catch { return false; }
      });
      if (!owns) return res.status(403).json({ error: "غير مسموح" });

      const checkRes = await ahminixCheckOrder(orderId);
      if (!checkRes?.data?.[0]) return res.json({ status: "processing", replay_api: [] });
      const ext = checkRes.data[0];

      // إذا انتهى → حدّث الطلب في قاعدة البيانات
      if (ext.status === "accept" || ext.status === "reject") {
        const newStatus = ext.status === "accept" ? "completed" : "cancelled";
        const matchOrder = (orders || []).find((o: any) => {
          try { return String(JSON.parse(o.meta || "{}").ahminix_order_id) === String(orderId); }
          catch { return false; }
        });
        if (matchOrder) {
          const oldMeta = JSON.parse(matchOrder.meta || "{}");
          await supabase.from("orders").update({
            status: newStatus,
            meta: JSON.stringify({ ...oldMeta, ahminix_status: ext.status, ahminix_replay: ext.replay_api || [] })
          }).eq("id", matchOrder.id);
          if (newStatus === "cancelled") {
            const { data: od } = await supabase.from("orders").select("total_amount").eq("id", matchOrder.id).single();
            if (od) await supabase.rpc("increment_balance", { user_id_param: userId, amount_param: od.total_amount });
          }
        }
      }
      res.json({ status: ext.status, replay_api: ext.replay_api || [] });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  });
