...
async function ahminixCreateOrder(
  ...
  const hasPlayerId = playerId && playerId.trim() !== "";

  let url: string;
  if (hasPlayerId) {
    // منتج يحتاج Player ID → endpoint /params
    const params = new URLSearchParams({
      qty: String(qty),
      order_uuid: orderUuid,
      playerId: playerId.trim()
    });
    url = `${AHMINIX_BASE}/newOrder/${productId}/params?${params.toString()}`;
  } else {
    // منتج لا يحتاج Player ID → endpoint /noParams
    const params = new URLSearchParams({
      qty: String(qty),
      order_uuid: orderUuid
    });
    url = `${AHMINIX_BASE}/newOrder/${productId}/noParams?${params.toString()}`;
  }
