...
import {
  ...
  authLimiter, forgotPassLimiter, orderLimiter, chatLimiter, uploadLimiter, generalLimiter,
  ...
  sanitizeText, isValidVoucherCode, isValidTxNumber, isValidUrl,
  requireSelf,
} from "./security.js";
...
async function startServer() {
  ...
  // Security Middleware (يجب أن تكون أول شيء)
  ...
  app.post("/api/orders", authenticate, orderLimiter, async (req, res) => {
    try {
      ...
      if (!isValidQuantity(quantity, 100)) return res.status(400).json({ error: "الكمية يجب أن تكون بين 1 و 100" });
  ...
  app.post("/api/admin/login", authLimiter, async (req, res) => {
    try {
      ...
      // مقارنة ثابتة الوقت لمنع Timing Attack
