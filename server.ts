...
async function startServer() {
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    ...
    try {
      // Re-verify code (one more time for security)
      const { data: otpRecord } = await supabase.from("email_verifications")
        .select("id, expires_at")
        .eq("email", email).eq("code", code).eq("type", "reset")
        .order("created_at", { ascending: false }).limit(1).single();
      // Allow recently-used OTP within 5 min (since verify-reset-otp already marked it used)
      if (!otpRecord || new Date(otpRecord.expires_at) < new Date()) {
        return res.status(400).json({ error: "انتهت صلاحية الرمز، ابدأ من جديد" });
      }
      const salt = await bcrypt.genSalt(10);
