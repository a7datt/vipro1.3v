...
async function startServer() {
  ...
  app.get("/api/admin/users/:id/details", async (req, res) => {
    try {
      const { data: stats } = await supabase.from("user_stats").select("referral_count, total_orders_count, total_recharge_sum").eq("user_id", req.params.id).single();
      res.json(stats || { referral_count: 0, total_orders_count: 0, total_recharge_sum: 0 });
