...
const BRAND_COLOR = process.env.BRAND_COLOR || "#B00000";
...
<div style="max-width:480px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
  ...
  <div style="padding:32px 24px;text-align:center">
    ...
    <div style="background:#f8f8f8;border:2px dashed ${BRAND_COLOR};border-radius:12px;padding:24px;margin:0 0 24px">
      <p style="margin:0 0 8px;font-size:13px;color:#888">رمز التحقق</p>
      <p style="margin:0;font-size:40px;font-weight:900;letter-spacing:12px;color:${BRAND_COLOR};font-family:monospace">${otp}</p>
    </div>
...
// ============================================================
// KAY (Admin Token) SYSTEM
// ============================================================
const ADMIN_EMAIL = "a7datt@gmail.com";
const ADMIN_PASSWORD_HASH = "$2a$10$"; // will be checked against DB

function generateKay(): string {
  const chars = "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789";
  let kay = "";
  for (let i = 0; i < 32; i++) {
    kay += chars[Math.floor(Math.random() * chars.length)];
  }
  return `KAY-${kay}`;
}

async function createNewKay(): Promise<string> {
  const kay = generateKay();
  const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(); // 7 days
  // Deactivate all old keys
  await supabase.from("admin_keys").update({ active: false }).eq("active", true);
  // Insert new key
  await supabase.from("admin_keys").insert({ kay, expires_at: expiresAt, active: true });
  // Email the new KAY
  await sendOtpEmail(ADMIN_EMAIL, kay, "reset"); // reuse email func with custom subject below
  // Actually send dedicated email
  if (RESEND_API_KEY) {
    ...
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: ADMIN_EMAIL,
        subject: "VIPro - رمز KAY الجديد لوحة التحكم",
        html: `<div dir="rtl" style="font-family:Arial;max-width:480px;margin:auto;padding:24px;background:#f9f9f9;border-radius:12px">
          <h2 style="color:${BRAND_COLOR}">رمز KAY الجديد</h2>
          <p>تم توليد رمز KAY جديد للوصول للوحة التحكم:</p>
          <div style="background:#fff;border:2px solid ${BRAND_COLOR};border-radius:8px;padding:16px;text-align:center;margin:16px 0">
            <code style="font-size:18px;font-weight:bold;letter-spacing:2px;color:${BRAND_COLOR}">${kay}</code>
          </div>
          <p style="color:#666;font-size:13px">صالح حتى: ${expiryDate}</p>
          <p style="color:#999;font-size:12px">لا تشارك هذا الرمز مع أحد.</p>
        </div>`
      })
    }).catch(console.error);
  }
  console.log(`[KAY] New admin key generated, expires: ${expiresAt}`);
  return kay;
}

async function validateKay(kay: string): Promise<boolean> {
  if (!kay) return false;
  const { data } = await supabase.from("admin_keys")
...
// Auto-rotate KAY every 7 days
async function autoRotateKay() {
  const { data } = await supabase.from("admin_keys")
    .select("id, expires_at")
    .eq("active", true)
    .single();
  if (!data || new Date(data.expires_at) < new Date()) {
    console.log("[KAY] Auto-rotating expired admin key...");
    await createNewKay();
  }
}

// ============================================================
// IN-MEMORY RATE LIMITER (no external packages)
// ============================================================
...
async function startServer() {
  ...
  // ============================================================
  // ADMIN KAY MIDDLEWARE - protects all /api/admin/* except login
  // ============================================================
  const adminAuth = async (req: any, res: any, next: any) => {
    const kay = req.headers["x-admin-kay"] as string;
    if (!kay) return res.status(401).json({ error: "غير مصرح - KAY مطلوب" });
    const valid = await validateKay(kay);
    if (!valid) return res.status(401).json({ error: "KAY منتهي الصلاحية أو غير صحيح" });
    next();
  };

  // Apply to all /api/admin/* EXCEPT login and kay-verify
  app.use("/api/admin", (req: any, res: any, next: any) => {
    const exempt = ["/api/admin/login", "/api/admin/kay/verify"];
    if (exempt.includes(req.path) || req.path === "/login" || req.path === "/kay/verify") return next();
    return adminAuth(req, res, next);
  });

  // KAY verify endpoint
  app.post("/api/admin/kay/verify", async (req, res) => {
    try {
      const { kay } = req.body;
      if (!kay) return res.status(400).json({ error: "KAY مطلوب" });
      const valid = await validateKay(kay);
      res.json({ valid });
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // KAY rotate endpoint (already authenticated via middleware)
  app.post("/api/admin/kay/rotate", async (req, res) => {
    try {
      const newKay = await createNewKay();
      res.json({ success: true, kay: newKay });
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // Start KAY auto-rotation check
  autoRotateKay().catch(console.error);
  setInterval(() => autoRotateKay().catch(console.error), 60 * 60 * 1000); // check every hour
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    if (!name || !email || !password) return res.status(400).json({ error: "البيانات غير مكتملة" });
    const ip = req.ip || req.connection.remoteAddress || "unknown";
    if (!rateLimit(`register:${ip}`, 5, 60 * 60 * 1000)) {
      return res.status(429).json({ error: "محاولات كثيرة، انتظر ساعة" });
    }
    try {
      ...
      console.log(`[REGISTER] OTP for ${email}: ${otp} (sent=${sent})`);
  ...
  app.post("/api/auth/verify-email", async (req, res) => {
    ...
    try {
      ...
      if (user.referred_by_id) {
        ...
        await supabase.from("users").update({ referral_count: (refUser?.referral_count || 0) + 1 }).eq("id", user.referred_by_id);
      }
  ...
  app.post("/api/auth/resend-otp", async (req, res) => {
    ...
    try {
      ...
      console.log(`[RESEND-OTP] ${email} type=${type} otp=${otp} sent=${sent}`);
  ...
  app.post("/api/auth/forgot-password", async (req, res) => {
    ...
    if (!email) return res.status(400).json({ error: "البريد مطلوب" });
    const ip = req.ip || req.connection.remoteAddress || "unknown";
    if (!rateLimit(`forgot:${ip}:${email}`, 3, 60 * 60 * 1000)) {
      return res.status(429).json({ error: "محاولات كثيرة، انتظر ساعة" });
    }
    try {
      ...
      console.log(`[FORGOT-PWD] OTP for ${email}: ${otp} (sent=${sent})`);
      res.json({ success: true });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    ...
    if (!email || !newPassword) return res.status(400).json({ error: "البيانات ناقصة" });
    try {
  ...
  app.post("/api/orders", async (req, res) => {
    const ip = req.ip || req.connection.remoteAddress || "unknown";
    if (!rateLimit(`orders:${ip}`, 20, 60 * 1000)) {
      return res.status(429).json({ error: "محاولات كثيرة، انتظر دقيقة" });
    }
    try {
      const { userId, productId, quantity, extraData } = req.body;
      if (!userId || !productId) return res.status(400).json({ error: "بيانات ناقصة" });
      if (typeof quantity !== "undefined" && (isNaN(Number(quantity)) || Number(quantity) < 0 || Number(quantity) > 100000)) {
  ...
  app.post("/api/admin/login", async (req, res) => {
    try {
      const ip = req.ip || req.connection.remoteAddress || "unknown";
      if (!rateLimit(`admin_login:${ip}`, 5, 15 * 60 * 1000)) {
        return res.status(429).json({ error: "محاولات كثيرة، انتظر 15 دقيقة" });
      }
      const { email, password } = req.body;
      // Check credentials
      ...
      const validPassword = password === (setting?.value || "ahmada7d1221");
      if (validEmail && validPassword) {
        // Get or create active KAY
        const { data: existingKey } = await supabase.from("admin_keys")
          .select("kay, expires_at")
          .eq("active", true)
          .single();
        let kay: string;
        if (existingKey && new Date(existingKey.expires_at) > new Date()) {
          ...
          kay = await createNewKay();
        }
        res.json({ success: true, kay });
      } else {
  ...
  app.post("/api/admin/users/:id/balance", async (req, res) => {
    try {
      const newBalance = req.body.amount ?? req.body.balance;
      if (newBalance === undefined || isNaN(parseFloat(newBalance))) {
        return res.status(400).json({ error: "المبلغ غير صالح" });
      }
      await supabase.from("users").update({ balance: parseFloat(newBalance) }).eq("id", req.params.id);
      res.json({ success: true });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  if (AHMINIX_TOKEN) {
      ...
      try {
        ...
        for (const order of processingOrders) {
          try {
            ...
            const newStatus = ext.status === "accept" ? "completed"
              : ext.status === "reject" ? "cancelled"
              : "processing";

            if (newStatus === "processing") continue;

            // Update order status + save replay_api codes
            const updatedMeta = {
              ...meta,
              ahminix_status: ext.status,
              ahminix_replay: ext.replay_api || []
            };
            await supabase.from("orders").update({
              status: newStatus,
              meta: JSON.stringify(updatedMeta)
            ...
            if (newStatus === "cancelled") {
              ...
              if (orderData) {
                await supabase.rpc("increment_balance", {
                  ...
                  amount_param: orderData.total_amount
                });
              }
            }
            ...
            if (userTg?.telegram_chat_id && userBot) {
              if (newStatus === "completed") {
                const codes = ext.replay_api?.filter(Boolean) || [];
                ...
                userBot.sendMessage(
                  userTg.telegram_chat_id,
                  `❌ تم رفض طلبك #ORD${order.id} وتم إعادة رصيدك.`
                ).catch(() => {});
              }
            }

            console.log(`[AUTO-REFRESH] Order #${order.id}: ${order.status} → ${newStatus}`);
          } catch (e: any) {
    ...
    console.log("[AUTO-REFRESH] Started - checking processing orders every 2 minutes");
  }
    ...
    userBot.on("message", async (msg: any) => {
      ...
      if (state.step === "login_with_code") {
        ...
        try {
          ...
          state.step = "register_verify_otp";
          userBot!.sendMessage(chatId,
            `📧 تم إرسال رمز تحقق من 6 أرقام إلى:\n${state.data.email}\n\nأدخل الرمز للتحقق من بريدك وإتمام التسجيل.\n\n💡 لم يصلك البريد؟ تحقق من مجلد Spam.`
          );
        } catch (e: any) {
          console.error(e);
          userBot!.sendMessage(chatId, "❌ فشل إرسال رمز التحقق. حاول مرة أخرى /start");
          userStates.delete(chatId);
        }

      } else if (state.step === "register_verify_otp") {
        const otpCode = text.trim();
        try {
          const valid = await verifyOtp(state.data.email, otpCode, "verify");
          if (!valid) {
            userBot!.sendMessage(chatId, "❌ الرمز غير صحيح أو منتهي الصلاحية. أرسل الرمز مرة أخرى، أو ابدأ من جديد /start");
            return;
          }
          ...
          await supabase.from("user_stats").insert({ user_id: user.id });
          if (referredById) {
            const { data: refUser } = await supabase.from("users").select("referral_count").eq("id", referredById).single();
            await supabase.from("users").update({ referral_count: (refUser?.referral_count || 0) + 1 }).eq("id", referredById);
          }
          userStates.delete(chatId);
          sendTelegramMessage(`👤 مستخدم جديد (بوت)\nالاسم: ${state.data.name}\nالإيميل: ${state.data.email}`);
          userBot!.sendMessage(chatId, "✅ تم التحقق من بريدك وإنشاء الحساب بنجاح!");
          sendMainMenu(chatId, user, userBot!);
        } catch (e: any) {
          console.error(e);
