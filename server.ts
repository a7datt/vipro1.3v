...
// تحويل replay_api لمصفوفة strings دائماً (الـ API قد يُرجع objects أو أرقام أو strings)
function normalizeReplayApi(raw: any): string[] {
  ...
  return arr
    .map((x: any) => {
      ...
      if (typeof x === "object") {
        // لو object فيه replay أو code أو value نأخذه، وإلا نحوله JSON
        const val = x.replay ?? x.code ?? x.value ?? x.key ?? null;
...
async function startServer() {
  ...
  app.post("/api/orders", authenticate, orderLimiter, async (req, res) => {
    try {
      ...
      if (hasExternalId && orderMode === 'auto') {
        ...
        console.log(`[API] Creating order: ext_id=${product.external_id}, qty=${qty}, playerId="${playerId}", uuid=${orderUuid}`);
        ...
        if (!ahminixRes || ahminixRes.status !== "OK") {
          ...
          // الـ API قد يرجع order_id رقمي أو UUID — نحفظ كليهما
          ahminixOrderId = String(ahminixRes.data?.order_id || orderUuid);
          ahminixOrderStatus = ahminixRes.data?.status || "processing";
          ahminixReplayApi = normalizeReplayApi(ahminixRes.data?.replay_api);
          // نحفظ الـ UUID دائماً كـ tracking key مستقل
          (ahminixRes as any)._uuid = orderUuid;
          console.log(`[API] Order created: id=${ahminixOrderId}, uuid=${orderUuid}, status=${ahminixOrderStatus}, replay=${ahminixReplayApi.length}`);
        }
      }
      // ====================================================================
      ...
      const metaData: any = {
        ...
        ...(ahminixOrderId ? {
          ...
          ahminix_order_uuid: (ahminixRes as any)?._uuid || ahminixOrderId,
  ...
  app.get("/api/orders/check-external/:orderId", authenticate, async (req, res) => {
    try {
      ...
      // جلب طلبات المستخدم قيد المعالجة
      ...
      try {
        ...
        for (const order of processingOrders) {
          try {
            ...
            console.log(`[AUTO-REFRESH] Order #${order.id}: processing → ${newStatus}, replay=${normalizedReplay.length}`);
          } catch (e: any) {
