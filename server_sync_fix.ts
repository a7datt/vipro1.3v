...
import {
  ...
  authLimiter, forgotPassLimiter, orderLimiter, chatLimiter, uploadLimiter, generalLimiter, strictLimiter,
...
async function startServer() {
  ...
  app.get("/api/referrals/stats/:userId", authenticate, async (req, res) => {
    ...
    try {
      ...
      // تحديث referral_count في users و user_stats ليكونا متزامنَين دائماً
  ...
  const autoSyncProducts = async () => {
    ...
    try {
      ...
      // نجلب السعر الحالي لحساب نسبة الربح المحفوظة وتطبيقها على السعر الجديد
      const { data: dbProducts } = await supabase
        .from("products")
        .select("id, external_id, available, name, price, price_per_unit, description, min_quantity")
      ...
      let updated = 0, markedUnavailable = 0, restored = 0, noChange = 0;
      ...
      for (const dbProd of (dbProducts || [])) {
        ...
        if (!apiProd) {
          // المنتج اختفى من API → نُعطّله فقط بدون حذف (لحماية بيانات الطلبات القديمة)
          if (dbProd.available) {
            await supabase.from("products")
              .update({ available: false })
              .eq("id", dbProd.id);
            markedUnavailable++;
            console.log(`[AUTO-SYNC] تعطيل (اختفى من API): ${dbProd.name}`);
          }
          continue;
        }

        const apiAvailable = apiProd.available !== false;
        const apiBasePrice  = parseFloat(apiProd.price) || 0;
        const apiName       = (apiProd.name || "").trim();
        const apiDesc       = (apiProd.category_name || apiProd.description || "").trim();
        const apiMinQty     = apiProd.qty_values?.min || apiProd.min_qty || 1;

        // ── حساب السعر الجديد مع الحفاظ على نسبة الربح المحفوظة ──
        // نسبة الربح = (سعر_الـDB / سعر_الـAPI_القديم) - 1
        // نُعيد تطبيقها على سعر API الجديد
        let newPrice = apiBasePrice;
        const dbPrice = parseFloat(dbProd.price) || parseFloat(dbProd.price_per_unit) || 0;
        if (dbPrice > 0 && apiBasePrice > 0) {
          // نستخرج نسبة الربح الحالية من السعر المخزون مقارنةً بسعر API
          // لكن سعر API القديم غير مخزون مباشرة، فنستخدم نسبة ثابتة:
          // إذا السعر في الـ DB أكبر من سعر API الجديد → نحافظ على الفرق النسبي
          // الصيغة: newPrice = apiBasePrice * (dbPrice / apiBasePrice_old)
          // بما أن apiBasePrice_old غير متوفر، نحافظ على نفس الفرق المطلق تقريباً:
          // نحسب markup_ratio من نسبة (dbPrice - apiOldPrice) / apiOldPrice
          // لكن الأبسط والأأمن: نحافظ على نفس السعر إذا تغيّر سعر API بنسبة أقل من 1%
          // وإذا تغيّر بأكثر → نُطبّق نفس نسبة الفرق
          // الحل: newPrice = apiBasePrice * (dbPrice / dbPrice) = apiBasePrice إذا لا markup
          // أو نحفظ الـ markup كـ: ratio = dbPrice / apiOldPrice
          // بما أننا لا نعرف apiOldPrice، نستخدم: نسبة الزيادة = dbPrice / apiBasePrice (الحالي)
          const markupRatio = dbPrice / apiBasePrice; // هذا يُعطي النسبة الحالية
          newPrice = parseFloat((apiBasePrice * markupRatio).toFixed(6));
          // إذا السعر الجديد نفسه السعر القديم (لم يتغير سعر الـ API) → لا تغيير
        } else {
          newPrice = parseFloat(apiBasePrice.toFixed(6));
        }

        // ── تحديد ما الذي تغيّر ──
        const nameChanged    = apiName && apiName !== dbProd.name;
        const descChanged    = apiDesc && apiDesc !== (dbProd.description || "");
        const minQtyChanged  = apiMinQty !== dbProd.min_quantity;
        const availChanged   = apiAvailable !== dbProd.available;
        // السعر: نتحقق فقط إذا تغيّر سعر API (نقارن newPrice بالسعر الحالي)
        const priceChanged   = Math.abs(newPrice - dbProd.price) > 0.000001;

        const hasChanges = nameChanged || descChanged || minQtyChanged || availChanged || priceChanged;
        ...
        // ── بناء object التحديث — لا نلمس: image_url, subcategory_id, sub_sub_category_id ──
        const updatePayload: Record<string, any> = {};

        if (availChanged)  updatePayload.available    = apiAvailable;
        if (nameChanged)   updatePayload.name         = apiName;
        if (descChanged)   updatePayload.description  = apiDesc;
        if (minQtyChanged) updatePayload.min_quantity = apiMinQty;
        if (priceChanged)  {
          updatePayload.price          = newPrice;
          updatePayload.price_per_unit = newPrice;
        }

        await supabase.from("products").update(updatePayload).eq("id", dbProd.id);

        if (!apiAvailable && dbProd.available) {
          markedUnavailable++;
          console.log(`[AUTO-SYNC] ❌ غير متوفر: ${apiName || dbProd.name}`);
        } else if (apiAvailable && !dbProd.available) {
          restored++;
          console.log(`[AUTO-SYNC] ✅ عاد متوفراً: ${apiName || dbProd.name}`);
        } else {
          updated++;
          const changes = [
            nameChanged   ? `اسم` : "",
            priceChanged  ? `سعر(${dbProd.price}→${newPrice})` : "",
            descChanged   ? `وصف` : "",
            minQtyChanged ? `حد_أدنى` : "",
          ].filter(Boolean).join(", ");
          console.log(`[AUTO-SYNC] 🔄 تحديث: ${apiName || dbProd.name} | ${changes}`);
        }
      }

      console.log(`[AUTO-SYNC] ✅ اكتملت: تحديث=${updated}, تعطيل=${markedUnavailable}, استعادة=${restored}, بدون_تغيير=${noChange}`);
    } catch (e: any) {
    ...
    userBot.on("message", async (msg: any) => {
      ...
      if (state.step === "login_with_code") {
        ...
        try {
          ...
          console.log(`[BOT-REGISTER] OTP for ${state.data.email}: ${otp} (sent=${sent})`);
