...
const REWARD_GOALS = [
  ...
  { id: 7, target: 500, title: "الهدف السابع",  icon: "🔥", rewardText: "شارة أسطورية + لقب \"الأسطورة الخالدة\" + جميع الثيمات + دعم خاص + خصم 5% مدى الحياة", rewards: { badge: 'legendary', title: 'الأسطورة الخالدة', anyTheme: true, specialSupport: true, discount: 5 } }
];
...
في حال كانت العملية بالليرة السورية سيتم تعبئة رصيد بـ1$ لكل 120 ل.س جديدو</p>
      ...
      <div className="px-4 space-y-5 pb-20">
        ...
        <div className={`bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center text-center space-y-3 ${user?.is_vip ? 'vip-card-glow border-amber-200 shadow-amber-50' : ''}`}>
          ...
          {user && (
            ...
            {/* زر الترتيب */}
            {user && (
              <button onClick={() => setView({ type: "leaderboard" })} className="w-full mt-2 bg-gradient-to-r from-amber-50 to-yellow-50 text-amber-700 py-2.5 rounded-2xl border border-amber-100 flex items-center justify-center gap-2 transition-all active:scale-95 font-bold text-sm">
  ...
  const LeaderboardView = () => {
    ...
    const tabs = [
      { key: 'topup',    label: 'أكثر شحناً',    icon: '💰', desc: 'إجمالي الشحن مدى الحياة' },
      { key: 'referral', label: 'أكثر إحالةً',   icon: '🔗', desc: 'أكثر من شارك رابط الإحالة' },
      { key: 'activity', label: 'الأكثر تفاعلاً', icon: '⚡', desc: 'أكثر المستخدمين نشاطاً' },
    ] as const;
    ...
    return (
      <div className="px-4 space-y-4 pb-20">
        <div className="flex items-center gap-3 pt-2">
          ...
          <div>
            <h2 className="font-bold text-gray-800 text-lg flex items-center gap-2"><Trophy size={20} className="text-amber-500" />لوحة الترتيب</h2>
            <p className="text-xs text-gray-400">ترتيب مدى الحياة</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {tabs.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`p-2.5 rounded-2xl border text-center transition-all active:scale-95 ${tab === t.key ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-white border-gray-100 text-gray-500'}`}>
              <div className="text-lg">{t.icon}</div>
              <p className={`text-[10px] font-bold mt-0.5 ${tab === t.key ? 'text-amber-700' : 'text-gray-500'}`}>{t.label}</p>
            </button>
          ))}
        </div>

        <p className="text-[10px] text-center text-gray-400">{tabs.find(t => t.key === tab)?.desc}</p>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {data.map((entry: any, i: number) => (
              <div key={i} className={`flex items-center gap-3 p-3 rounded-2xl border ${i === 0 ? 'bg-amber-50 border-amber-200' : i === 1 ? 'bg-gray-50 border-gray-200' : i === 2 ? 'bg-orange-50 border-orange-100' : 'bg-white border-gray-100'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {entry.badge && <span className="text-[9px] text-amber-600 font-bold">{entry.badge}</span>}
                </div>
                <div className="text-right shrink-0">
                  <p className={`font-black text-sm ${i < 3 ? 'text-amber-600' : 'text-gray-700'}`}>{entry.value}</p>
        ...
        {user && data.findIndex((e:any) => String(e.user_id) === String(user.id)) === -1 && data.length > 0 && (
          <div className="bg-gray-50 rounded-2xl p-3 text-center border border-gray-100">
            <p className="text-xs text-gray-400">أنت لست في الترتيب الأول حتى الآن. استمر!</p>
          </div>
        )}
      </div>
    );
...
const AdminHomeTab = ({adminUsers, adminOrders, adminTransactions, setAdminTab, fetchUser}: any) => {
...
const AdminOrdersTab = ({adminOrders, handleOrderAction, orderSearch, setOrderSearch, orderDateFilter, setOrderDateFilter, fetchAdminOrders}: any) => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 pb-20 text-right" dir="rtl" data-admin-panel="true">
        ...
        <div className="p-4 space-y-4">
          {/* ===== ADMIN HOME ===== */}
          {adminTab === "admin_home" && <AdminHomeTab adminUsers={adminUsers} adminOrders={adminOrders} adminTransactions={adminTransactions} setAdminTab={setAdminTab} fetchUser={fetchUser} />}

          {adminTab === "chat" && <AdminChatView />}

          {/* ===== ORDERS ===== */}
          {adminTab === "orders" && <AdminOrdersTab adminOrders={adminOrders} handleOrderAction={handleOrderAction} orderSearch={orderSearch} setOrderSearch={setOrderSearch} orderDateFilter={orderDateFilter} setOrderDateFilter={setOrderDateFilter} fetchAdminOrders={fetchAdminOrders} />}
