...
في حال كانت العملية بالليرة السورية سيتم تعبئة رصيد بـ1$ لكل 120 ل.س جديدو</p>
  ...
  const ForgotPasswordView = () => {
    ...
    if (success) {
      return (
        <div className="px-6 flex flex-col items-center justify-center min-h-[80vh] pb-20 text-center">
          <div className="w-20 h-20 bg-green-100 rounded-3xl flex items-center justify-center mb-8">
            <ShieldCheck size={40} className="text-green-600" />
          </div>
          <h2 className="text-2xl font-bold text-gray-800 mb-2">تم تغيير كلمة المرور!</h2>
          <p className="text-gray-400 text-sm mb-8">يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة.</p>
          <button onClick={() => setView({ type: "login" })} className="w-full bg-brand text-white py-4 rounded-2xl font-bold shadow-lg shadow-brand-soft">
            تسجيل الدخول
          </button>
        </div>
      );
    ...
    if (sent && !verifiedCode) {
      return (
        <VerifyEmailView
          email={email}
          type="reset"
          onSuccess={() => setVerifiedCode("verified")}
        />
      );
    }
    ...
    if (verifiedCode) {
      return (
        <div className="px-6 flex flex-col items-center justify-center min-h-[80vh] pb-20">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">كلمة مرور جديدة</h2>
          <p className="text-gray-400 text-sm mb-8 text-center">أدخل كلمة المرور الجديدة لحسابك</p>
          <div className="w-full space-y-4">
            ...
            <button disabled={resetting} onClick={handleResetPassword} className="w-full bg-brand text-white py-4 rounded-2xl font-bold shadow-lg shadow-brand-soft disabled:opacity-50">
    ...
    return (
      <div className="px-6 flex flex-col items-center justify-center min-h-[80vh] pb-20">
        ...
        <p className="text-gray-400 text-sm mb-8 text-center">أدخل بريدك الإلكتروني وسنرسل لك رمز التحقق</p>
        <div className="w-full space-y-4">
          <input type="email" placeholder="البريد الإلكتروني" value={email} onChange={e => setEmail(e.target.value)}
            className="w-full bg-white border border-gray-100 rounded-2xl px-5 py-4 outline-none focus:border-brand shadow-sm"/>
          {error && <p className="text-red-500 text-sm text-center">{error}</p>}
          <button disabled={loading} onClick={handleSendOtp} className="w-full bg-brand text-white py-4 rounded-2xl font-bold shadow-lg shadow-brand-soft disabled:opacity-50">
            {loading ? "جاري الإرسال..." : "إرسال رمز التحقق"}
          </button>
          <button onClick={() => setView({ type: "login" })} className={`w-full ${theme.text} text-sm font-bold pt-2`}>
        ...
        <div className="w-full space-y-4">
          ...
          {!isRegister && (
            <button
              ...
              className="w-full text-gray-400 text-sm font-bold pt-1"
            >
              نسيت كلمة المرور؟
            </button>
          )}
