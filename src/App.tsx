...
const REWARD_GOALS = [
  { id: 1, target: 5, title: "الهدف الأول", rewardText: "وسام البداية + لقب \"عضو ناشئ\" + إطار حساب برونزي + خصم 1% لمدة شهر", rewards: { badge: 'bronze', title: 'ناشئ', frame: 'bronze', discount: 1 } },
  { id: 2, target: 15, title: "الهدف الثاني", rewardText: "شارة النشاط + لقب \"عضو نشيط\" + إطار حساب فضي + 1$ رصيد + خصم 2% لمدة شهر", rewards: { badge: 'active', title: 'نشيط', frame: 'silver', balance: 1, discount: 2 } },
  { id: 3, target: 30, title: "الهدف الثالث", rewardText: "شارة الطاقة + لقب \"عضو متميز\" + 3$ رصيد + خصم 3% لمدة شهر + كوبون 10%", rewards: { badge: 'energy', title: 'متميز', balance: 3, discount: 3 } },
  { id: 4, target: 50, title: "الهدف الرابع", rewardText: "شارة فضية + لقب \"عضو VIP\" + إطار VIP + 5$ رصيد + خصم 4% لمدة سنة + أولوية في الطلبات", rewards: { badge: 'silver', title: 'VIP', frame: 'vip', balance: 5, discount: 4, priority: true } },
  { id: 5, target: 100, title: "الهدف الخامس", rewardText: "تاج ذهبي + لقب \"نجم المتجر\" + إطار ذهبي متحرك + 5$ رصيد + خصم 5% + كوبون 15% + شارة فضية", rewards: { badge: 'gold', title: 'نجم', frame: 'gold_animated', balance: 5, discount: 5, theme: 'yellow' } },
  { id: 6, target: 200, title: "الهدف السادس", rewardText: "شارة الماس + لقب \"أسطورة\" + 10$ رصيد + خصم 5% + كوبون 15% + شارة ذهبية + دعم خاص", rewards: { badge: 'diamond', title: 'أسطورة', balance: 10, discount: 5, theme: 'red', specialSupport: true } },
  { id: 7, target: 500, title: "الهدف السابع", rewardText: "لقب الاسطورة + 20$ رصيد + خصم 5% + كوبون 20% + شارة أسطورية نادرة + جميع الثيمات + دعم خاص على مدار الساعة", rewards: { badge: 'legendary', title: 'لقب الاسطورة', anyTheme: true, balance: 20, discount: 5, customCodes: true, specialSupport: true } }
];
    ...
    return (
      <div className="px-4 space-y-6 pb-20">
        <div className={`bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center text-center space-y-4 ${user?.is_vip ? 'vip-card-glow border-amber-200 shadow-amber-50' : ''}`}>
          ...
          {/* دفعاتي - فوق نظام المكافآت */}
          {user && (
            <button
              onClick={() => setView({ type: "payments" })}
              className="w-full bg-white border border-gray-100 shadow-sm rounded-2xl p-4 flex items-center justify-between hover:border-brand-soft transition-all"
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-green-50 rounded-xl flex items-center justify-center">
                  <History size={20} className="text-green-600" />
                </div>
                <span className="font-bold text-gray-700">دفعاتي</span>
              </div>
      ...
      <div className="min-h-screen bg-gray-50 pb-20 text-right" dir="rtl" data-admin-panel="true">
        ...
        <div className="p-4 space-y-4">
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {filtered.map(u => {
                    const [showNotifFor2, setShowNotifFor2] = React.useState(false);
                    const [nt2, setNt2] = React.useState("");
                    const [nb2, setNb2] = React.useState("");
                    return (
                      <div key={u.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        ...
                        <div className="border-t border-gray-50 px-3 py-2.5 flex items-center gap-2 flex-wrap">
                          ...
                          <button onClick={() => setShowNotifFor2(!showNotifFor2)}
                        ...
                        {showNotifFor2 && (
                          <div className="border-t border-blue-50 bg-blue-50/40 px-4 py-3 space-y-2">
                            ...
                            <input type="text" placeholder="عنوان الإشعار" value={nt2} onChange={e => setNt2(e.target.value)} className="w-full p-2.5 bg-white rounded-xl text-xs outline-none border border-blue-100"/>
                            <input type="text" placeholder="نص الإشعار" value={nb2} onChange={e => setNb2(e.target.value)} className="w-full p-2.5 bg-white rounded-xl text-xs outline-none border border-blue-100"/>
                            <button onClick={async () => { if (!nt2 || !nb2) return; await handleSendNotification(u.id, nt2, nb2); setNt2(""); setNb2(""); setShowNotifFor2(false); }}
                    ...
                    <div key={t.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-3">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{t.payment_method_name} · {new Date(t.created_at).toLocaleDateString("ar-EG")}</p>
                      ...
                      {t.receipt_url && <img src={t.receipt_url} className="w-full h-32 object-cover rounded-xl" referrerPolicy="no-referrer"/>}
                      {t.status === 'pending' && (
                        <div className="flex gap-2">
          ...
          {adminTab === "ahminix" && (() => {
            ...
            const [targetSubcategoryId, setTargetSubcategoryId] = React.useState("");
            const [targetSubSubCategoryId, setTargetSubSubCategoryId] = React.useState("");
            ...
            const loadProfile = async () => { setLoadingProfile(true); const res = await fetch("/api/admin/ahminix/profile"); setAhminixProfile(await res.json()); setLoadingProfile(false); };
            const loadProducts = async () => { setLoadingProducts(true); const res = await fetch("/api/admin/ahminix/products"); const data = await res.json(); setAhminixProducts(Array.isArray(data)?data:[]); setLoadingProducts(false); };
              ...
              <div className="space-y-6 pb-8">
                ...
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={loadProducts} disabled={loadingProducts} className="bg-gray-100 text-gray-700 px-3 py-2 rounded-xl text-xs font-bold disabled:opacity-50 flex items-center gap-1"><RefreshCw size={12} className={loadingProducts?"animate-spin":""}/>{loadingProducts?"...":"جلب المنتجات"}</button>
                  </div>

                  <div className="space-y-3">
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">التصنيف</p>
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">القسم الرئيسي</label>
                      <select onChange={e => setTargetSubcategoryId("")} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none" id="import-cat-sel">
                    ...
                    <div>
                      ...
                      <select value={targetSubcategoryId} onChange={e => { setTargetSubcategoryId(e.target.value); setTargetSubSubCategoryId(""); }} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none">
                  ...
                  {ahminixProducts.length > 0 && (
                    <div className="space-y-3 border-t border-gray-100 pt-4">
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">اختر المنتجات</p>
                      <div className="flex gap-2">
                        <div className="relative flex-1"><Search size={12} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"/><input type="text" placeholder="بحث..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full bg-gray-50 border border-gray-100 rounded-xl pr-8 pl-3 py-2 text-xs outline-none"/></div>
                        <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="bg-gray-50 border border-gray-100 rounded-xl px-3 py-2 text-xs outline-none"><option value="">الكل</option>{uniqueCategories.map((c:any) => <option key={c} value={c}>{c}</option>)}</select>
                      </div>
                      <div className="flex items-center justify-between">
                        <button onClick={() => setSelectedProducts(filteredProducts.map(p=>p.id))} className="text-xs text-brand font-bold">تحديد الكل</button>
                        <button onClick={() => setSelectedProducts([])} className="text-xs text-gray-400 font-bold">إلغاء الكل</button>
                      </div>
                      <div className="space-y-2 max-h-64 overflow-y-auto">
                        {filteredProducts.map((p:any) => (
                          <div key={p.id} onClick={() => toggleProduct(p.id)} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${selectedProducts.includes(p.id)?'border-[#B00000] bg-red-50':'border-gray-100 bg-white hover:border-gray-200'}`}>
                            <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${selectedProducts.includes(p.id)?'border-[#B00000] bg-[#B00000]':'border-gray-300'}`}>{selectedProducts.includes(p.id)&&<CheckCircle size={10} className="text-white"/>}</div>
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-bold text-gray-800 truncate">{p.name}</p>
                              <p className="text-[10px] text-gray-400">{p.category_name} · {p.price} $</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const res = await fetch("/api/admin/ahminix/sync", { method:"POST", headers:{"Content-Type":"application/json"},
                          body: JSON.stringify({ subcategoryId:parseInt(targetSubcategoryId), subSubCategoryId:targetSubSubCategoryId?parseInt(targetSubSubCategoryId):undefined, productIds:selectedProducts.length?selectedProducts:undefined, markupPercent, productOverrides:productOverrides.length?productOverrides:undefined })
                        });
                  ...
                  {syncResult && (
                    <div className={`p-4 rounded-xl text-sm ${syncResult.error?'bg-red-50 text-red-700':'bg-green-50 text-green-700'}`}>
                      {syncResult.error?`خطأ: ${syncResult.error}`:`مضاف: ${syncResult.summary?.added} · محدّث: ${syncResult.summary?.updated} · تجاوز: ${syncResult.summary?.skipped}`}
                    </div>
                  )}
                </div>
              </div>
        ...
        {/* Add Modals */}
        <AnimatePresence>
          {activeSubMenu && activeSubMenu.startsWith("add_") && activeSubMenu !== "add_balance" && activeSubMenu !== "add_notification" && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] bg-black/50 flex items-end justify-center" onClick={() => setActiveSubMenu(null)}>
              <motion.div initial={{y:100,opacity:0}} animate={{y:0,opacity:1}} exit={{y:100,opacity:0}} className="bg-white w-full max-w-lg rounded-t-3xl p-6 space-y-4 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-bold text-gray-800">{activeSubMenu==="add_category"?"إضافة قسم رئيسي":activeSubMenu==="add_subcategory"?"إضافة قسم فرعي":activeSubMenu==="add_subSubCategory"?"إضافة فرع فرعي":activeSubMenu==="add_voucher"?"إضافة قسيمة":"إضافة طريقة دفع"}</h3>
                  <button onClick={() => setActiveSubMenu(null)} className="text-gray-400"><X size={22}/></button>
                </div>
        ...
        <AnimatePresence>
          {activeSubMenu === "add_notification" && (() => {
            const [nt2, setNt2] = React.useState<"all"|"one">("all");
            const [nuid2, setNuid2] = React.useState("");
            const [ntitle2, setNtitle2] = React.useState("");
            const [nbody2, setNbody2] = React.useState("");
            const [nsending2, setNsending2] = React.useState(false);
            return (
              <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] bg-black/50 flex items-end" onClick={() => setActiveSubMenu(null)}>
                <motion.div initial={{y:80}} animate={{y:0}} exit={{y:80}} className="bg-white w-full rounded-t-3xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
                  <h3 className="font-bold text-gray-800 flex items-center gap-2"><Bell size={18}/>إرسال إشعار</h3>
                  <div className="flex gap-2">
                    <button onClick={() => setNt2("all")} className={`flex-1 py-2 rounded-xl text-xs font-bold ${nt2==="all"?"bg-[#B00000] text-white":"bg-gray-100 text-gray-600"}`}>للجميع</button>
                    <button onClick={() => setNt2("one")} className={`flex-1 py-2 rounded-xl text-xs font-bold ${nt2==="one"?"bg-[#B00000] text-white":"bg-gray-100 text-gray-600"}`}>لمستخدم معين</button>
                  </div>
                  {nt2==="one" && <input type="text" placeholder="ID المستخدم" className="w-full p-3 bg-gray-50 rounded-xl text-sm outline-none" value={nuid2} onChange={e => setNuid2(e.target.value)}/>}
                  <input type="text" placeholder="عنوان الإشعار" className="w-full p-3 bg-gray-50 rounded-xl text-sm outline-none" value={ntitle2} onChange={e => setNtitle2(e.target.value)}/>
                  <textarea placeholder="نص الإشعار" className="w-full p-3 bg-gray-50 rounded-xl text-sm outline-none h-20 resize-none" value={nbody2} onChange={e => setNbody2(e.target.value)}/>
                  <button disabled={nsending2||!ntitle2||!nbody2} onClick={async () => { setNsending2(true); await handleSendNotification(nt2==="one"&&nuid2?parseInt(nuid2):null,ntitle2,nbody2); setNsending2(false); setActiveSubMenu(null); }}
                    ...
                    <Send size={16}/>{nsending2?"جاري الإرسال...":"إرسال الإشعار"}
                  </button>
                </motion.div>
              </motion.div>
            );
