          ...
          {checkResult && (
            <div className="bg-gray-50 p-4 rounded-xl">
              {checkResult.error ? (
                ...
                const statusLabel = d.status === "accept" || d.status === "completed" ? "مقبول ✅" : d.status === "reject" || d.status === "rejected" || d.status === "cancelled" ? "مرفوض ❌" : "قيد المعالجة ⏳";
                const statusColor = d.status === "accept" || d.status === "completed" ? "text-green-700 bg-green-100" : d.status === "reject" || d.status === "rejected" || d.status === "cancelled" ? "text-red-700 bg-red-100" : "text-amber-700 bg-amber-100";
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (d.error) {
                          ...
                          const apiSt = d.ahminixStatus || "";
                          const apiLabel = apiSt === "completed" || apiSt === "accept" ? "مكتمل ✅" : apiSt === "rejected" || apiSt === "reject" || apiSt === "cancelled" ? "مرفوض ❌" : `قيد المعالجة (${apiSt})`;
                          const dbLabel = d.newStatus === "completed" ? "مكتمل ✅" : d.newStatus === "cancelled" ? "ملغي ❌" : d.newStatus;
                          alert(`API: ${apiLabel}\nحالة الطلب في النظام: ${dbLabel}`);
                        }
                        fetchAdminOrders();
                      }}
