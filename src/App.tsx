...
const AdminUserCard = ({ u, fetchAdminUsers, handleToggleVip, handleBlockUser, handleDeleteUser, handleSendNotification, onOpenChat, onViewOrders, onViewTransactions }: any) => {
  ...
  const handleSendDirectChat = async () => {
    ...
    const res = await adminFetch(`/api/admin/chat/send`, {
      method: "POST",
      headers: {"Content-Type":"application/json"},
      body: JSON.stringify({ userId: u.id, message: chatMsg.trim() })
    });
    if (res.ok) { setChatMsg(""); setShowChat(false); alert("✅ تم إرسال الرسالة"); }
    else alert("❌ فشل إرسال الرسالة");
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      {/* Chat Panel */}
      {showChat && (
        <div className="border-t border-indigo-50 bg-indigo-50/40 px-4 py-3 space-y-2">
          <p className="text-[11px] font-bold text-indigo-700">إرسال رسالة لـ {u.name}</p>
          <textarea value={chatMsg} onChange={e => setChatMsg(e.target.value)}
            placeholder="اكتب رسالتك هنا..."
            className="w-full p-2.5 bg-white rounded-xl text-xs outline-none border border-indigo-100 resize-none h-20"/>
          <div className="flex gap-2">
            <button onClick={handleSendDirectChat}
              className="flex-1 bg-indigo-600 text-white py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1">
              <Send size={12}/> إرسال
            </button>
            <button onClick={() => onOpenChat && onOpenChat(u)}
              className="flex-1 bg-indigo-50 text-indigo-700 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1 border border-indigo-100">
