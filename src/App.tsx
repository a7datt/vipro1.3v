...
في حال كانت العملية بالليرة السورية سيتم تعبئة رصيد بـ1$ لكل 120 ل.س جديدو</p>
  ...
  const AdminAhminixTab = ({ categories, subcategories, subSubCategories, fetchCategories, fetchSubcategories, fetchSubSubCategories }: any) => {
    ...
    const handleCheckOrder = async () => { if (!checkOrderId.trim()) return; setCheckLoading(true); setCheckResult(null); try { const raw = checkOrderId.trim(); const isUuid = raw.includes("-"); // إذا أدخل رقماً فقط أو بادئة ID_ نتعامل معه كـ numeric ID const normalizedId = (!isUuid && !raw.startsWith("ID_")) ? `ID_${raw}` : raw; const res = await adminFetch(`/api/admin/ahminix/check-order/${encodeURIComponent(normalizedId)}${isUuid?"?uuid=1":""}`); setCheckResult(await res.json()); } catch(e:any){setCheckResult({error:e.message});} setCheckLoading(false); };
  ...
  React.useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        // الـ API يُرجع: accept, reject, أو processing/pending
