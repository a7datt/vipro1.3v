      ...
      <div className="border-t border-gray-50 px-3 py-2.5 flex items-center gap-2 flex-wrap">
        <button onClick={async () => {
          ...
          if (nb2 !== null && !isNaN(parseFloat(nb2))) {
            const res = await adminFetch(`/api/admin/users/${u.id}/balance`, { method: "POST", headers: {"Content-Type":"application/json", "Authorization": `Bearer ${localStorage.getItem("adminToken")||""}`}, body: JSON.stringify({balance: parseFloat(nb2)}) });
