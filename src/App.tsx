...
في حال كانت العملية بالليرة السورية سيتم تعبئة رصيد بـ1$ لكل 120 ل.س جديدو</p>
    ...
    return (
      <div className="px-4 space-y-6 pb-20">
        <div className={`bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center text-center space-y-4 ${user?.is_vip ? 'vip-card-glow border-amber-200 shadow-amber-50' : ''}`}>
          ...
          {/* حالة ربط تليجرام */}
          {user && (
            <div className={`w-full rounded-2xl border p-4 flex items-center justify-between ${user.telegram_chat_id ? 'bg-blue-50 border-blue-100' : 'bg-amber-50 border-amber-100'}`}>
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${user.telegram_chat_id ? 'bg-blue-100' : 'bg-amber-100'}`}>
                  <MessageSquare size={18} className={user.telegram_chat_id ? 'text-blue-600' : 'text-amber-600'} />
                </div>
                <div>
                  <p className={`text-sm font-bold ${user.telegram_chat_id ? 'text-blue-800' : 'text-amber-800'}`}>
                    {user.telegram_chat_id ? 'تليجرام مرتبط' : 'تليجرام غير مرتبط'}
                  </p>
                  <p className={`text-[10px] ${user.telegram_chat_id ? 'text-blue-500' : 'text-amber-500'}`}>
                    {user.telegram_chat_id ? 'حسابك مؤمن ومرتبط بالبوت' : 'اربط حسابك للحصول على إشعارات'}
                  </p>
                </div>
              </div>
              {user.telegram_chat_id ? (
                <button onClick={handleUnlinkTelegram} className="text-[10px] font-bold text-red-500 bg-red-50 px-3 py-1.5 rounded-xl border border-red-100 active:scale-95 transition-all">فك الربط</button>
              ) : (
                <button onClick={handleGenerateLinkingCode} className="text-[10px] font-bold text-blue-600 bg-white px-3 py-1.5 rounded-xl border border-blue-200 active:scale-95 transition-all">ربط الآن</button>
              )}
            </div>
          )}
  ...
  const AdminAhminixTab = ({ categories, subcategories, subSubCategories, fetchCategories, fetchSubcategories, fetchSubSubCategories }: any) => {
    ...
    const loadProducts = async () => { setLoadingProducts(true); try { const res = await fetch("/api/admin/ahminix/products"); const data = await res.json(); setAhminixProducts(Array.isArray(data)?data:[]); } catch { setAhminixProducts([]); } setLoadingProducts(false); };
            ...
            onClick={async () => {
              ...
              try {
                const res = await fetch("/api/admin/ahminix/sync", { method:"POST", headers:{"Content-Type":"application/json"},
                  body: JSON.stringify({ subcategoryId:parseInt(targetSubcategoryId), subSubCategoryId:targetSubSubCategoryId?parseInt(targetSubSubCategoryId):undefined, productIds:selectedProducts.length?selectedProducts:undefined, markupPercent })
                });
