        ...
        <div className="px-4 space-y-6 pb-20 flex flex-col items-center justify-center min-h-[60vh]">
          ...
          <div className="w-full bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-3">
            ...
            {orderResult.externalStatus === 'processing' && (() => {
              ...
              React.useEffect(() => {
                if (!orderResult.externalOrderId) return;
                let cancelled = false;
                let attempts = 0;
                ...
                const interval = setInterval(async () => {
                  ...
                  setPolling(true);
                  try {
                    const res = await fetch(`/api/admin/ahminix/check-order/${encodeURIComponent(orderResult.externalOrderId)}`);
                    const data = await res.json();
                    if (data?.data?.[0]) {
                      ...
                      if (ext.status === 'accept' || ext.status === 'reject') {
                        ...
                        setOrderResult((prev: any) => ({
                          ...
                          replayApi: ext.replay_api || prev.replayApi
                        }));
                        fetchOrders();
                      }
                    }
