...
const AdminLoginView = ({ setIsAdmin, setAdminAuth, setView }: AdminLoginViewProps) => {
  const [email, setEmail] = useState("a7datt@gmail.com");
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res.ok && data.kay) {
        localStorage.setItem("admin_kay", data.kay);
        setIsAdmin(true);
        setAdminAuth(true);
        setView({ type: "main" });
      } else {
        setError(data.error || "بريد أو كلمة مرور خاطئة");
      }
    } catch (e: any) {
      setError(e.name === 'TypeError' ? "فشل الاتصال بالسيرفر" : "حدث خطأ غير متوقع");
    } finally {
      setLoading(false);
    }
  };
  return (
    <div className="px-6 flex flex-col items-center justify-center min-h-[70vh] space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-400 text-sm">بريد الإدمن وكلمة المرور</p>
      </div>
      <div className="w-full space-y-4">
        <input
          type="email"
...
// Admin fetch helper - automatically adds KAY header
function adminFetch(url: string, options: RequestInit = {}): Promise<Response> {
  const kay = localStorage.getItem("admin_kay") || "";
...
export default function App() {
  ...
  const BottomNav = () => {
    const processingOrders = orders.filter(o => o.status === 'processing' || o.status === 'pending_admin').length;
    const unreadMessages = user?.unread_support_count || 0;
    return (
      <nav className="fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-gray-100 flex items-center justify-around z-40">
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-1 relative ${activeTab === "orders" ? theme.text : "text-gray-400"}`}
        >
          <div className="relative">
            ...
            {processingOrders > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">
                {processingOrders > 9 ? "9+" : processingOrders}
              </span>
            )}
          </div>
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-1 relative ${activeTab === "profile" && view.type === "chat" ? theme.text : "text-gray-400"}`}
        >
          <div className="relative">
          ...
          <span className="text-[10px] font-medium">الدعم</span>
        </button>
        <button 
          onClick={() => { setActiveTab("profile"); setView({ type: "main" }); }}
          ...
          {categories.length === 0 ? (
            [1,2,3,4,5,6].map(i => (
              <div key={i} className="bg-gray-100 rounded-xl overflow-hidden animate-pulse">
                ...
                <div className="h-5 bg-gray-200 rounded mx-2 my-2"/>
              </div>
            ))
          ) : categories.map(cat => (
            <motion.button 
              ...
              className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col items-center overflow-hidden transition-colors active:scale-95"
            >
              <div className="w-full aspect-square overflow-hidden">
                {cat.image_url
                  ? <img src={cat.image_url} alt={cat.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  : <div className={`w-full h-full ${theme.bgLight} flex items-center justify-center`}><ShoppingBag size={28} className={theme.icon}/></div>
                }
              </div>
              <span className="font-bold text-gray-700 text-[10px] text-center py-2 px-1 w-full">{cat.name}</span>
            </motion.button>
          ))}
        </div>
      </div>
  ...
  const CheckoutView = () => {
    ...
    const handlePurchase = async () => {
      ...
      if (prod.requires_input && prod.store_type !== 'external_api' && !extraData) return setError("يرجى إدخال البيانات المطلوبة");
      if (prod.requires_input && prod.store_type === 'external_api' && !extraData) return setError("يرجى إدخال البيانات المطلوبة (Player ID / رقم الهاتف)");
      ...
      return (
        <div className="px-4 space-y-6 pb-20 flex flex-col items-center justify-center min-h-[60vh]">
          ...
          <div className="w-full bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-3">
            ...
            <div className="flex justify-between text-sm"><span className="text-gray-500">المبلغ المدفوع</span><span className="font-bold text-brand">{finalPrice.toFixed(2)} $</span></div>
            {orderResult.replayApi?.length > 0 && (
              <div className="bg-gray-50 p-3 rounded-xl">
                ...
                {orderResult.replayApi.map((r: string, i: number) => (
                  <p key={i} className="text-sm font-mono font-bold text-brand break-all">{r}</p>
                ))}
              </div>
            )}
    ...
    return (
      <div className="px-4 space-y-6 pb-20">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          ...
          {/* حقل Player ID للمنتجات الخارجية (Ahminix) - يظهر دائماً لضمان إرسال البيانات */}
          {prod.store_type === 'external_api' && (
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-700">
                {prod.description && prod.description.toLowerCase().includes('phone') ? 'رقم الهاتف' :
                 prod.description && prod.description.toLowerCase().includes('email') ? 'البريد الإلكتروني' :
                 prod.requires_input ? 'معرف اللاعب / البيانات المطلوبة' : 'معرف اللاعب (اختياري)'}
              </label>
              <input
                ref={extraRef}
                type="text"
...
في حال كانت العملية بالليرة السورية سيتم تعبئة رصيد بـ1$ لكل 120 ل.س جديدو</p>
  ...
  const OrdersView = () => {
    ...
    const getOrderCodes = (metaStr: string): string[] => {
      try {
        ...
        const codes: string[] = [];
        // replay_api from Ahminix - the activation codes
        if (Array.isArray(meta.ahminix_replay)) codes.push(...meta.ahminix_replay.filter(Boolean));
        if (Array.isArray(meta.replay_api)) codes.push(...meta.replay_api.filter(Boolean));
        return [...new Set(codes)];
      } catch { return []; }
    };
    const getMetaLabels = (metaStr: string) => {
      // Keys to hide from user (internal fields)
      const hidden = new Set(['storeType','order_mode','ahminix_order_id','ahminix_status','ahminix_replay','replay_api','input','playerId']);
      try {
        const meta = JSON.parse(metaStr || "{}");
            ...
            <div key={order.id} className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
              ...
              {expandedOrderId === order.id && (
                <div className="px-4 pb-4 pt-2 border-t border-gray-50 space-y-3 bg-gray-50/50">
                  ...
                  {/* Auto-refresh for processing API orders */}
                  {order.status === 'processing' && (() => {
                    try {
                      const meta = JSON.parse(order.meta || "{}");
                      if (!meta.ahminix_order_id) return null;
                      return (
                        <OrderPollingStatus
                          externalOrderId={meta.ahminix_order_id}
                          onStatusChange={(_newStatus: string, replayApi: any[]) => {
                            fetchOrders();
                          }}
                        />
                      );
  ...
  const ProfileView = () => {
    ...
    return (
      <div className="px-4 space-y-5 pb-20">
        ...
        {/* زر KAY للإدمن فقط */}
        {user?.email === "a7datt@gmail.com" && (() => {
          const [showKay, setShowKay] = React.useState(false);
          const [loadingRotate, setLoadingRotate] = React.useState(false);
          const storedKay = localStorage.getItem("admin_kay") || "";
          return (
            <div className="bg-gray-900 rounded-2xl border border-gray-700 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-amber-400 flex items-center gap-2">
                  <ShieldCheck size={14}/> رمز KAY — لوحة التحكم
                </p>
                <button onClick={() => setShowKay(!showKay)} className="text-[10px] text-gray-400 bg-gray-800 px-2 py-1 rounded-lg active:scale-95">
                  {showKay ? "إخفاء" : "عرض KAY"}
                </button>
              </div>
              {showKay && (
                <div className="space-y-2">
                  <div className="flex items-center gap-2 bg-gray-800 rounded-xl p-2.5">
                    <code className="text-[10px] text-amber-300 font-mono flex-1 break-all">{storedKay || "—"}</code>
                    {storedKay && (
                      <button onClick={() => { navigator.clipboard.writeText(storedKay); }} className="text-[9px] bg-amber-500 text-black px-2 py-1 rounded-lg font-bold shrink-0">نسخ</button>
                    )}
                  </div>
                  <button
                    disabled={loadingRotate}
                    onClick={async () => {
                      if (!confirm("هل تريد توليد KAY جديد؟ سيُرسل للبريد الإلكتروني.")) return;
                      setLoadingRotate(true);
                      try {
                        const res = await adminFetch("/api/admin/kay/rotate", { method: "POST" });
                        const data = await res.json();
                        if (data.kay) {
                          localStorage.setItem("admin_kay", data.kay);
                          alert("✅ تم توليد KAY جديد وإرساله لبريدك الإلكتروني");
                        }
                      } catch { alert("خطأ في الاتصال"); }
                      setLoadingRotate(false);
                    }}
                    className="w-full bg-red-900 text-red-300 py-2 rounded-xl text-xs font-bold disabled:opacity-50 active:scale-95"
                  >
                    {loadingRotate ? "جاري التغيير..." : "🔄 تغيير KAY"}
                  </button>
                </div>
              )}
            </div>
        ...
        <div className="px-6 flex flex-col items-center justify-center min-h-[80vh] pb-20">
          ...
          <div className="w-full space-y-4">
            ...
            <button disabled={resetting} onClick={handleResetPassword}
              ...
              {resetting ? "جاري الحفظ..." : "حفظ كلمة السر وتسجيل الدخول"}
            </button>
          </div>
        </div>
      );
      ...
      <div className="fixed inset-0 z-[60] bg-gray-50 flex flex-col bottom-16">
            ...
            {messages.map((m) => (
              ...
              >
                <div className={`max-w-[85%] p-3 rounded-2xl shadow-sm relative ${
                  m.sender_role === 'user' ? 'bg-white text-gray-800 rounded-tr-none' : 'bg-brand text-white rounded-tl-none'
                }`}>
                  {m.type === 'bot_reply' && (
                  ...
                  {m.type === 'rating_request' ? (
                    <div className="text-center py-2">
                      ...
                      <button 
                        ...
                        className="w-full py-2 bg-white text-brand rounded-xl text-xs font-bold active:scale-95 transition-all disabled:opacity-50"
                      >
                        تأكيد التقييم
                      </button>
                    </div>
        ...
        <div className="p-4 bg-white border-t border-gray-100 flex items-center gap-2 shrink-0">
          ...
          <input 
            ...
            className="flex-1 bg-gray-50 border-none rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 focus:ring-brand transition-all"
            value={newMessage}
  ...
  const AdminChatView = () => {
    ...
    const fetchChatList = async () => {
      const res = await adminFetch("/api/admin/chat/list");
      const data = await res.json();
      ...
      return (
        <div className="fixed inset-0 z-[60] bg-gray-50 flex flex-col bottom-16">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar bg-[#F7F7F7]">
            <AnimatePresence initial={false}>
              {messages.map((m) => (
                ...
                >
                  <div className={`max-w-[85%] p-3 rounded-2xl shadow-sm relative ${
                    m.sender_role === 'admin' ? 'bg-brand text-white rounded-tr-none' : 'bg-white text-gray-800 rounded-tl-none'
                  }`}>
          ...
          <div className="p-4 bg-white border-t border-gray-100 flex items-center gap-2 sticky bottom-0">
            ...
            />
            <button onClick={() => handleSendReply(reply)} className="p-3 bg-brand text-white rounded-xl shadow-lg shadow-red-100 active:scale-95 transition-all">
    ...
    return (
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] bg-black/50 flex items-end" onClick={onClose}>
        <motion.div initial={{y:80}} animate={{y:0}} exit={{y:80}} className="bg-white w-full rounded-t-3xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex gap-2">
            <button onClick={() => setTarget("all")} className={`flex-1 py-2 rounded-xl text-xs font-bold ${target==="all"?"bg-brand text-white":"bg-gray-100 text-gray-600"}`}>للجميع</button>
            <button onClick={() => setTarget("one")} className={`flex-1 py-2 rounded-xl text-xs font-bold ${target==="one"?"bg-brand text-white":"bg-gray-100 text-gray-600"}`}>لمستخدم معين</button>
          </div>
          ...
          <button disabled={sending||!title||!body} onClick={async () => { setSending(true); await handleSendNotification(target==="one"&&uid?parseInt(uid):null, title, body); setSending(false); onClose(); }}
            className="w-full bg-brand text-white py-3.5 rounded-2xl font-bold text-sm disabled:opacity-40 flex items-center justify-center gap-2">
  ...
  const AdminAhminixTab = ({ categories, subcategories, subSubCategories, fetchCategories, fetchSubcategories, fetchSubSubCategories }: any) => {
    ...
    const loadProfile = async () => { setLoadingProfile(true); try { const res = await adminFetch("/api/admin/ahminix/profile"); setAhminixProfile(await res.json()); } catch(e:any) { setAhminixProfile({error:e.message}); } setLoadingProfile(false); };
    const loadProducts = async () => { setLoadingProducts(true); setProductsFetched(false); setSelectedProducts([]); setSyncResult(null); try { const res = await adminFetch("/api/admin/ahminix/products"); const data = await res.json(); setAhminixProducts(data.products || (Array.isArray(data)?data:[])); setProductsFetched(true); } catch { setAhminixProducts([]); setProductsFetched(true); } setLoadingProducts(false); };
    ...
    return (
      <div className="space-y-6 pb-8">
        <div className="bg-gradient-to-r from-brand to-[#8B0000] p-6 rounded-2xl text-white">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-gray-800 flex items-center gap-2"><Wallet size={18} className="text-brand"/>حساب API الخارجي</h3>
            <button onClick={loadProfile} disabled={loadingProfile} className="bg-brand text-white px-4 py-2 rounded-xl text-xs font-bold disabled:opacity-50 flex items-center gap-1"><RefreshCw size={12} className={loadingProfile?"animate-spin":""}/>{loadingProfile?"جاري...":"تحديث"}</button>
          </div>
          {ahminixProfile ? (
        ...
        {/* تحديث الطلبات */}
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          <h3 className="font-bold text-gray-800 flex items-center gap-2"><RefreshCw size={18} className="text-brand"/>تحديث حالة الطلبات</h3>
          <p className="text-xs text-gray-500">يفحص الطلبات بحالة "processing" ويحدّثها تلقائياً</p>
          <button onClick={handleRefreshOrders} disabled={refreshLoading} className="w-full bg-brand text-white py-3 rounded-xl font-bold disabled:opacity-50 flex items-center justify-center gap-2"><RefreshCw size={16} className={refreshLoading?"animate-spin":""}/>{refreshLoading?"جاري التحديث...":"تحديث الطلبات"}</button>
        ...
        {/* فحص طلب */}
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          <h3 className="font-bold text-gray-800 flex items-center gap-2"><Search size={18} className="text-brand"/>فحص طلب</h3>
          <div className="flex gap-2">
            ...
            <button onClick={handleCheckOrder} disabled={checkLoading} className="bg-brand text-white px-4 py-3 rounded-xl font-bold text-sm disabled:opacity-50">{checkLoading?"...":"فحص"}</button>
          </div>
          {checkResult && (
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-gray-800 flex items-center gap-2"><Download size={18} className="text-brand"/>استيراد المنتجات</h3>
            <button onClick={loadProducts} disabled={loadingProducts} className="bg-brand text-white px-4 py-2 rounded-xl text-xs font-bold disabled:opacity-50 flex items-center gap-1"><RefreshCw size={12} className={loadingProducts?"animate-spin":""}/>{loadingProducts?"جاري الجلب...":"جلب المنتجات"}</button>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <select value={targetSubcategoryId} onChange={e => { setTargetSubcategoryId(e.target.value); setTargetSubSubCategoryId(""); }} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none focus:border-brand">
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto rounded-xl border border-gray-100">
                {filteredProducts.length === 0 ? (
                  ...
                  <div key={p.id} onClick={() => toggleProduct(p.id)}
                    className={`flex items-center gap-3 p-3 cursor-pointer transition-all ${selectedProducts.includes(p.id)?'bg-red-50 border-r-2 border-brand':'bg-white hover:bg-gray-50'}`}>
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${selectedProducts.includes(p.id)?'border-brand bg-brand':'border-gray-300'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-[10px] font-bold text-brand">{p.price} $</span>
          ...
          <button
            ...
            className="w-full bg-brand text-white py-3.5 rounded-xl font-bold disabled:opacity-40 flex items-center justify-center gap-2"
          >
...
const OrderPollingStatus = ({ externalOrderId, onStatusChange }: { externalOrderId: string; onStatusChange: (status: string, replayApi: any[]) => void }) => {
  ...
  React.useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      setPolling(true);
      try {
        const res = await adminFetch(`/api/admin/ahminix/check-order/${encodeURIComponent(externalOrderId)}`);
        const data = await res.json();
        if (data?.data?.[0]) {
      ...
      <div className="border-t border-gray-50 px-3 py-2.5 flex items-center gap-2 flex-wrap">
        <button onClick={async () => {
          ...
          if (nb2 !== null && !isNaN(parseFloat(nb2))) {
            const res = await adminFetch(`/api/admin/users/${u.id}/balance`, { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({balance: parseFloat(nb2)}) });
...
const AdminOrdersTab = ({adminOrders, orderSearch, setOrderSearch, orderDateFilter, setOrderDateFilter, fetchAdminOrders}: any) => {
  ...
  setModeLoading(true);
  await adminFetch("/api/admin/settings", { method: "POST", headers: { "Content-Type": "application/json", "x-admin-kay": localStorage.getItem("admin_kay") || "" }, body: JSON.stringify({ key: "order_processing_mode", value: newMode }) });
  setOrderMode(newMode);
  setModeLoading(false);
  };
  const handleOrderAction = async (orderId: number, action: "approved" | "rejected", adminResp?: string) => {
  await adminFetch(`/api/admin/orders/${orderId}/status`, { method: "POST", headers: { "Content-Type": "application/json", "x-admin-kay": localStorage.getItem("admin_kay") || "" }, body: JSON.stringify({ status: action, admin_response: adminResp || "" }) });
  fetchAdminOrders();
  };
  ...
  <div className="space-y-4">
    <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h3 className="font-bold text-gray-800 flex items-center gap-2"><Zap size={16} className="text-brand"/>وضع المعالجة</h3>
        ...
        {modeLoaded && (
          <div className="flex bg-gray-100 rounded-xl p-1 gap-1">
            ...
            <button onClick={() => toggleMode("manual")} disabled={modeLoading} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${orderMode === 'manual' ? 'bg-brand text-white shadow-sm' : 'text-gray-500'}`}>يدوي</button>
          </div>
        )}
      </div>
    </div>
        ...
        {pendingAdminOrders.map(order => {
          ...
          return (
            <div key={order.id} className="border border-amber-100 rounded-xl bg-amber-50/40 p-4 space-y-3">
              <div>
                <p className="font-bold text-sm text-brand">#{order.id} - {order.product_name}</p>
        ...
        <div key={order.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xs font-bold text-brand">{(order.total_price || 0).toFixed(2)} $</p>
            </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {[{val:"categories",label:"الأقسام"},{val:"subcategories",label:"الفرعية"},{val:"subSubCategories",label:"الفرع الفرعي"},{val:"products",label:"المنتجات"}].map(opt => (
              <button key={opt.val} onClick={() => { setElementsType(opt.val); loadElements(opt.val); }}
                className={`shrink-0 px-3 py-2 rounded-xl text-xs font-bold transition-all ${elementsType===opt.val?'bg-brand text-white':'bg-white border border-gray-100 text-gray-600'}`}>{opt.label}</button>
            ))}
          </div>
        )}
      ...
      {editingItem && (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-4" onClick={() => setEditingItem(null)}>
          <motion.div initial={{scale:0.92,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.92,opacity:0}} className="bg-white w-full max-w-md rounded-3xl p-6 space-y-4 max-h-[85vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={handleSaveEdit} className="w-full bg-brand text-white py-3.5 rounded-2xl font-bold text-sm mt-2">حفظ التعديلات</button>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
      ...
      <div className="min-h-screen bg-gray-50 pb-20 text-right" dir="rtl" data-admin-panel="true">
        ...
        {activeSubMenu === "fab_open" && (
          <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} exit={{opacity:0,y:30}}
            className="fixed bottom-20 left-4 right-4 z-[56] bg-white rounded-3xl shadow-2xl border border-gray-100 p-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                { key:"add_balance", label:"شحن رصيد", icon:<Wallet size={20}/>, color:"text-green-600 bg-green-50" },
                { key:"add_voucher", label:"قسيمة", icon:<Ticket size={20}/>, color:"text-purple-600 bg-purple-50" },
                { key:"add_paymentMethod", label:"طريقة دفع", icon:<CreditCard size={20}/>, color:"text-orange-600 bg-orange-50" },
                ...
                <button key={item.key}
                  onClick={() => { if(item.key==="ahminix_link"){setAdminTab("ahminix");setActiveSubMenu(null);}else setActiveSubMenu(item.key); }}
                  className="flex flex-col items-center gap-1.5 p-2.5 rounded-2xl hover:bg-gray-50 transition-colors active:scale-95">
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${item.color}`}>{item.icon}</div>
                  <span className="font-bold text-gray-600 text-[9px] text-center leading-tight">{item.label}</span>
                </button>
              ))}
            </div>
          </motion.div>
              ...
              <motion.div initial={{y:100,opacity:0}} animate={{y:0,opacity:1}} exit={{y:100,opacity:0}} className="bg-white w-full max-w-lg rounded-t-3xl p-6 space-y-4 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                {activeSubMenu === "add_category" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleAddCategory(); setActiveSubMenu(null); }} className="w-full bg-brand text-white py-3.5 rounded-2xl font-bold text-sm">إضافة القسم الرئيسي</button>
                  </div>
                )}
                {activeSubMenu === "add_subcategory" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleAddSubcategory(); setActiveSubMenu(null); }} className="w-full bg-brand text-white py-3.5 rounded-2xl font-bold text-sm">إضافة القسم الفرعي</button>
                  </div>
                )}
                {activeSubMenu === "add_subSubCategory" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleAddSubSubCategory(); setActiveSubMenu(null); }} className="w-full bg-brand text-white py-3.5 rounded-2xl font-bold text-sm">إضافة الفرع الفرعي</button>
                  </div>
                )}
                {activeSubMenu === "add_voucher" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleCreateVoucher(); setActiveSubMenu(null); }} className="w-full bg-brand text-white py-3.5 rounded-2xl font-bold text-sm">إنشاء القسيمة</button>
                  </div>
                )}
                {activeSubMenu === "add_paymentMethod" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleAddPaymentMethod(); setActiveSubMenu(null); }} className="w-full bg-brand text-white py-3.5 rounded-2xl font-bold text-sm">إضافة طريقة الدفع</button>
                  </div>
                )}
              </motion.div>
            </motion.div>
          ...
          {activeSubMenu === "add_balance" && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] bg-black/50 flex items-end" onClick={() => setActiveSubMenu(null)}>
              <motion.div initial={{y:80}} animate={{y:0}} exit={{y:80}} className="bg-white w-full rounded-t-3xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={() => { handleManualTopup(); setActiveSubMenu(null); }} className="w-full bg-brand text-white py-3.5 rounded-2xl font-bold text-sm">شحن الرصيد</button>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {([
            ...
            if (item.tab === "fab") return (
              <button key="fab" onClick={() => setActiveSubMenu(activeSubMenu==="fab_open"?null:"fab_open")}
                className="relative -top-4 w-14 h-14 bg-brand text-white rounded-full shadow-lg shadow-red-200 flex items-center justify-center active:scale-95 transition-all">
            ...
            return (
              <button key={item.tab} onClick={() => { setAdminTab(item.tab); setActiveSubMenu(null); }}
                className={`flex flex-col items-center gap-0.5 relative ${adminTab===item.tab?"text-brand":"text-gray-400"}`}>
                {item.icon}
                <span className="text-[9px] font-medium">{item.label}</span>
                {item.badge > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-brand text-white text-[8px] font-bold rounded-full flex items-center justify-center">{item.badge}</span>}
              </button>
            );
          })}
        </nav>
    ...
    <div className="min-h-screen bg-gray-50 text-right" dir="rtl">
      ...
      <AnimatePresence>
        {pageLoading && (
          ...
          >
            <div className="w-12 h-12 rounded-full border-4 border-gray-100 border-t-brand animate-spin" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
