...
في حال كانت العملية بالليرة السورية سيتم تعبئة رصيد بـ1$ لكل 120 ل.س جديدو</p>
    ...
    return (
      <div className="px-4 space-y-5 pb-20">
        ...
        <div className={`bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center text-center space-y-3 ${user?.is_vip ? 'vip-card-glow border-amber-200 shadow-amber-50' : ''}`}>
          ...
          {/* شريط الرصيد */}
          {user && (
            <div className={`w-full ${theme.bgLight} rounded-2xl border ${theme.border} p-3 flex items-center justify-between`}>
              <span className={`text-sm font-bold ${theme.textDark}`}>الرصيد الحالي</span>
              <span className={`text-lg font-black ${theme.text}`}>{user.balance?.toFixed(2) ?? "0.00"} $</span>
            </div>
          )}
                ...
                return (
                  <div key={goal.id} className={`space-y-3 p-4 rounded-2xl border ${isClaimed ? 'bg-brand-light border-brand-soft' : isCurrent ? 'bg-amber-50 border-amber-100' : 'bg-gray-50 border-gray-100 opacity-50'}`}>
                    <div className="flex justify-between items-start">
                      ...
                      {isClaimed ? (
                        <span className="flex items-center gap-1 text-[10px] font-bold text-brand"><CheckCircle size={14} />تم الاستلام</span>
                      ) : isReached ? (
                        <button
                          onClick={async () => {
                            try {
                        ...
                        >الحصول على المكافأة</button>
                      ) : (
                        <span className="text-[10px] font-bold text-amber-600">متبقي {(goal.target - user.stats!.total_recharge_sum).toFixed(2)} $</span>
                      )}
                    </div>
                    {!isClaimed && (
                      <div className="space-y-1.5">
                        ...
                        <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                          <div className={`h-full transition-all duration-500 ${isReached ? 'bg-brand' : 'bg-amber-500'}`} style={{ width: `${progress}%` }} />
                        </div>
                      </div>
                    )}
                  </div>
  ...
  const AdminAhminixTab = ({ categories, subcategories, subSubCategories, fetchCategories, fetchSubcategories, fetchSubSubCategories }: any) => {
    ...
    const loadProducts = async () => { setLoadingProducts(true); setProductsFetched(false); setSelectedProducts([]); setSyncResult(null); try { const res = await fetch("/api/admin/ahminix/products"); const data = await res.json(); setAhminixProducts(Array.isArray(data)?data:[]); setProductsFetched(true); } catch { setAhminixProducts([]); setProductsFetched(true); } setLoadingProducts(false); };
