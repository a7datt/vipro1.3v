/**
 * ════════════════════════════════════════════════════════════════
 *  security.ts  —  14 طبقة حماية متقدمة
 * ════════════════════════════════════════════════════════════════
 */
...
// ══════════════════════════════════════════════════════════════
//  الطبقة 2+3 — Rate Limiting + IP Blacklist الديناميكي
// ══════════════════════════════════════════════════════════════
...
// تنظيف strikes كل 30 دقيقة — يعطي فرصة ثانية للمستخدمين العاديين
setInterval(() => {
  strikeMap.clear();
}, 30*60*1000);
...
export function strikeIp(ip: string, reason: string, banOnStrikes = 15): void {
  const current = (strikeMap.get(ip) || 0) + 1;
  strikeMap.set(ip, current);
  if (current >= banOnStrikes) {
    // الحظر الأقصى 30 دقيقة فقط حتى لا يُظلم المستخدم العادي
    const duration = Math.min(current * 2 * 60 * 1000, 30*60*1000);
...
export function rateLimit(maxRequests: number, windowMs: number, opts?: { keyFn?: (req:Request)=>string; banAfterExceeds?: number }) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (entry.count > maxRequests) {
      // يعطي strike فقط إذا تجاوز الحد بفارق كبير جداً (الهجوم الحقيقي)
      const banThreshold = opts?.banAfterExceeds ?? maxRequests * 10;
      if (entry.count > banThreshold) strikeIp(ip, `rate-limit: ${req.path}`, 15);
...
// authLimiter: 20 محاولة/15 دقيقة — يحظر فقط عند 200 تجاوز (هجوم واضح)
export const authLimiter       = rateLimit(20,  15*60*1000, { banAfterExceeds:200 });
// forgotPass: 10/ساعة — يحظر فقط عند 80 تجاوز
export const forgotPassLimiter = rateLimit(10,  60*60*1000, { banAfterExceeds:80  });
// orderLimiter: 30/دقيقة
export const orderLimiter      = rateLimit(30,  60*1000);
// chatLimiter: 60/دقيقة
export const chatLimiter       = rateLimit(60,  60*1000);
// uploadLimiter: 15/دقيقة
export const uploadLimiter     = rateLimit(15,  60*1000);
// generalLimiter: 300/دقيقة — يحظر فقط عند 2000 تجاوز (DDoS واضح)
export const generalLimiter    = rateLimit(300, 60*1000, { banAfterExceeds:2000 });
// strictLimiter: 30/دقيقة — يحظر فقط عند 300 تجاوز
export const strictLimiter     = rateLimit(30,  60*1000, { banAfterExceeds:300 });
...
export function botDetection(req: Request, res: Response, next: NextFunction) {
  ...
  const ip = getClientIp(req);
  // بدون UA على API — strike منخفض، لا حظر فوري
  if (!ua && req.path.startsWith("/api/")) { strikeIp(ip,"no-ua",15); return res.status(400).json({ error:"طلب غير مقبول" }); }
  for (const p of BLOCKED_UA) if (p.test(ua)) { strikeIp(ip,`bot:${ua.slice(0,30)}`,10); return res.status(403).json({ error:"ممنوع" }); }
  next();
}
...
export function injectionGuard(req: Request, res: Response, next: NextFunction) {
  ...
  if (deepScan(req.body) || deepScan(req.query) || deepScan(req.params)) {
    // strike عالي — يحتاج 15 محاولة injection حقيقية قبل الحظر
    strikeIp(ip, "injection", 15);
...
export function corsMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  if (allowed.includes(origin) || !origin) {
    ...
    strikeIp(getClientIp(req), `cors:${origin}`, 20);
...
export function authenticate(req: Request, res: Response, next: NextFunction) {
  ...
  // strike عالي — 20 محاولة JWT خاطئة قبل الحظر
  if (!payload) { strikeIp(getClientIp(req),"invalid-jwt",20); return res.status(401).json({ error:"الجلسة منتهية، يرجى تسجيل الدخول مجدداً" }); }
...
export function adminAuth(req: Request, res: Response, next: NextFunction) {
  ...
  if (!payload || payload.role!=="admin") {
    // admin strike أعلى قليلاً لأنه هجوم مقصود لكن لا يزال يحتاج محاولات كثيرة
    strikeIp(getClientIp(req), "invalid-admin-jwt", 10);
...
export function requireSelf(paramName="userId") {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!requestedId || tokenId!==requestedId) {
      // IDOR يحتاج 20 محاولة قبل strike
      strikeIp(getClientIp(req), "idor-attempt", 20);
